...
client.login("schawncik tokencikk") ///TOKENİ BURAYA GİRİCEN AQ 
client.on("ready",async() =>{
    console.log("schâwn system | bota giriş başarılı")
});
(async () => {
    ...
    client.on("message",async(message) =>{
        ...
        if(message.guild && message.channel.id == "KANAL ID"  && message.content == ".whatsapp" && message.author.id == "KULLANACAK KİŞİ ID"){
            ...
            msg.edit("Lütfen birazcikkk beklermisin.")
            await page.goto('https://web.whatsapp.com/')
            msg.edit("Lütfen azcikk kaldi beklermisinn..")
            await page.waitForSelector('canvas')
            msg.edit("Bekla lan işte amk evladiiii al öptümmm")
            //bekle(1)
            ...
            await msg.edit('Lütfen QR Kodu tarat. (\`15 Saniye\` Kaldı)')
            ...
            await msg.edit('Lütfen QR Kodu **tekrar** tarat. (\`10 Saniye\` Kaldı)')
            bekle(5)
            await msg.edit('Lütfen QR Kodu **tekrar** tarat. (\`5 Saniye\` Kaldı)')
            bekle(2)
            await msg.edit('Lütfen QR Kodu **tekrar** tarat. (\`3 Saniye\` Kaldı)')
            bekle(1)
            await msg.edit('Lütfen QR Kodu **tekrar** tarat. (\`2 Saniye\` Kaldı)')
            bekle(1)
            await msg.edit('Lütfen QR Kodu **tekrar** tarat. (\`1 Saniye\` Kaldı)')
            bekle(1)
            await qrcode.delete()
            ...
            let checkLogin = await page.evaluate(v => {
                try{
                    ...
                    console.log(err)
                    return "https://github.com/schawnrabbializm"
                }
            }, checkLoginx);
            if(checkLogin !== "https://github.com/schawnrabbializm"){
                await msg.edit('Giriş yapıldı.')
                await msg.edit('Kim için Stalk yapmak istiyorsun? (Kayıtlı kişi ismi gir (`20 Saniye.` Kaldı))').then(async f => {
                    ...
                    message.channel.awaitMessages(filter, {
                      ...
                      errors: ['time']
                        ...
                        const checkUserx = `span[title="${kişi}"]`;      
                         ...
                         if(checkUser !== "https://github.com/schawnrabbializm"){
                            ...
                            msg.edit(`Kişi izlemeye konuldu. Lütfen DM İzinlerinin Açık Olduğundan Emin Ol. Artık DM kutuna veriler aktarılacak.`)
                            return running;
                            async function f(){
                                try{
                                    ...
                                    const dp_url = dpDiv ? dpDiv.src : 'https://cdn.discordapp.com/attachments/935914002654048256/1010237842473099314/whatsapp-schawn.png';
                                    console.log(`
                                    Name: ${name}
                                    StatusDiv: ${statusDiv}
                                    dpDiv: ${dpDiv}
